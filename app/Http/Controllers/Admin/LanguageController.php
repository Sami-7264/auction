...
class LanguageController extends Controller
{
    ...
    public function langSearch(Request $request)
    {
        ...
        if (array_key_exists($key, $json_arr)) {
            ...
            $notify[] = ['error', 'Your search key not found.'];
            return back()->withNotify($notify);
        }
        ...
        $offset = ($page - 1) * $perPage;
        $paginatedData = array_slice($json_arr, $offset, $perPage, true);
        $paginatedData = new LengthAwarePaginator($paginatedData, count($json_arr), $perPage, $page);
        $paginatedData->setPath(url()->current());
